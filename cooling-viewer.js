/* <lm-cooling-viewer> — Longmotive chiller-room inspector.
   Loads the real BIM export uploads/3d/cooling-room.glb (level-3 chiller room:
   2 water-cooled chillers, 3 CHW pump sets, buffer tank, CHW + condenser headers
   on a steel pipe rack). Raycast hits group by CR_* mesh-name families, so a click
   selects the whole equipment item. Free-orbit room viewer. */
(function(){
const ESM='https://cdn.jsdelivr.net/npm/three@0.160.0';
const GLB='uploads/3d/cooling-room.glb';
const POSTER='uploads/3d/cooling-room-hero.png';
const SEC={
  CH1:{name:'CHILLER 1',type:'Water-cooled chiller',devices:'Evaporator & condenser barrels · control panel'},
  CH2:{name:'CHILLER 2',type:'Water-cooled chiller',devices:'Evaporator & condenser barrels · control panel'},
  P1:{name:'CHW PUMP 1',type:'Chilled-water pump set',devices:'Suction & discharge spools on inertia base'},
  P2:{name:'CHW PUMP 2',type:'Chilled-water pump set',devices:'Suction & discharge spools on inertia base'},
  P3:{name:'CHW PUMP 3',type:'Chilled-water pump set',devices:'Suction & discharge spools on inertia base'},
  TK:{name:'BUFFER TANK',type:'Chilled-water buffer vessel',devices:'Vertical tank on housekeeping pad'},
  HS:{name:'CHW HEADERS',type:'Chilled-water supply & return',devices:'CHWS / CHWR headers + chiller tie-ins'},
  HC:{name:'CW HEADERS',type:'Condenser-water supply & return',devices:'CW headers to cooling towers'},
  RK:{name:'PIPE RACK',type:'Steel support structure',devices:'Columns, tie beams & pipe arms'}
};
const IDS=Object.keys(SEC);
const MAP=[
  [/^CR_(CH1_|PadChiller1|Lbl_CH1)/,'CH1'],
  [/^CR_(CH2_|PadChiller2|Lbl_CH2)/,'CH2'],
  [/^CR_(Pump1_|PadPump1|Lbl_P1)/,'P1'],
  [/^CR_(Pump2_|PadPump2|Lbl_P2)/,'P2'],
  [/^CR_(Pump3_|PadPump3|Lbl_P3)/,'P3'],
  [/^CR_(Tank_|PadTank|Lbl_TK)/,'TK'],
  [/^CR_(Header_CHW|Tie_CHW|Lbl_HB|Lbl_HG)/,'HS'],
...
class LMCoolingViewer extends HTMLElement{
  connectedCallback(){
  ...
  _fallback(e){
    console.error('Cooling viewer:',e);
...
if(!customElements.get('lm-cooling-viewer'))customElements.define('lm-cooling-viewer',LMCoolingViewer);
})();
