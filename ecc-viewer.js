/* <lm-ecc-viewer> — Longmotive ECC monitoring-room inspector.
   Loads the real BIM export uploads/3d/ecc-monitoring-room.glb (ECC room: tiled
   video wall, 2 dual-workstation operator desks, fire-command console, wall
   cabinets). Raycast hits group by ECC_* mesh-name families, so a click selects
...
const POSTER='uploads/3d/ecc-room-hero-3740d932.png';
const SEC={
  VW:{name:'VIDEO WALL',type:'Campus monitoring display',devices:'LED tile array on glow frame'},
  D1:{name:'OPERATOR DESK 1',type:'Dual-workstation console',devices:'2 workstations \u00b7 monitors, PCs & peripherals'},
  D2:{name:'OPERATOR DESK 2',type:'Dual-workstation console',devices:'2 workstations \u00b7 monitors, PCs & peripherals'},
  FC:{name:'FIRE COMMAND',type:'Fire-alarm command console',devices:'FACP cabinet \u00b7 monitor & keyboard'}
};
const IDS=Object.keys(SEC);
const MAP=[
  [/^ECC_VW_/,'VW'],
  [/^ECC_(Desk1_|Chair_O)/,'D1'],
  [/^ECC_(Desk2_|Chair_B)/,'D2'],
  [/^ECC_FC_/,'FC']
];
const HUDCFG={views:[{id:'overview',label:'Overview',key:'i'},{id:'plan',label:'Plan',key:'p'}],
  walls:true,colour:true,explode:true,poster:POSTER,loadingLabel:'Loading ECC room',
  ...
  _init(THREE,GLTFLoader,RoomEnvironment){
    ...
    new GLTFLoader().load(GLB,(gltf)=>{
      ...
      root.traverse(o=>{
        ...
        if(o.name==='ECC_CeilSlab'||o.name==='ECC_Baffles'){o.visible=false;return;} // roof slab off — light strips stay
      ...
      // model is authored open: only the video-wall wall + front skirt ship in the GLB
      ...
      // brand the screens: logo planes just in front of the 4 WS monitors + the video wall
      new THREE.TextureLoader().load('uploads/2-b83b68ab.jpg',(tex)=>{
        tex.colorSpace=THREE.SRGBColorSpace;
        const roomC=new THREE.Box3().setFromObject(root).getCenter(new THREE.Vector3());
        const vwM=root.getObjectByName('ECC_VW_Tiles');
        const vwC=vwM?new THREE.Box3().setFromObject(vwM).getCenter(new THREE.Vector3()):roomC;
        const AR=749/291;
        const brand=(mesh,frac,ref)=>{
          const bb=new THREE.Box3().setFromObject(mesh);
          const sz=bb.getSize(new THREE.Vector3()),c=bb.getCenter(new THREE.Vector3());
          const axis=sz.x<sz.z?'x':'z';           // thin axis = screen normal
          let w=(axis==='x'?sz.z:sz.x)*frac;
          const h=Math.min(w/AR,sz.y*frac); w=h*AR; // fit while keeping logo aspect
          const dir=(c[axis]-ref[axis])>=0?1:-1;    // face away from the reference point
          const pl=new THREE.Mesh(new THREE.PlaneGeometry(w,h),new THREE.MeshBasicMaterial({map:tex,toneMapped:false}));
          pl.position.copy(c);
          pl.position[axis]=dir>0?bb.max[axis]+.006:bb.min[axis]-.006;
          pl.rotation.y=axis==='z'?(dir>0?0:Math.PI):(dir>0?Math.PI/2:-Math.PI/2);
          scene.add(pl);
        };
        root.traverse(o=>{if(o.isMesh&&/^ECC_Desk\d_WS\d_Monitor$/.test(o.name))brand(o,.6,vwC);});
        if(vwM)brand(vwM,.4,roomC.clone().sub(vwC).multiplyScalar(-1).add(vwC)); // wall faces the room
        dirty=true;
      });
      this._applyVis=(fn)=>{
    ...
    const VIEWS={
      overview:{t:C.clone(),r:16,th:2.2,ph:1.1},
      plan:{t:C.clone(),r:19,th:2.2,ph:.28}
    };
    ...
    const TH_MIN=1.45,TH_MAX=2.95; // orbit window — camera never goes behind the wall
    this._goView=(v)=>{
