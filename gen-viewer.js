/* <lm-gen-viewer> — Longmotive generator-plant inspector.
   Loads the real BIM export uploads/3d/generator-room.glb (generator room: 3 standby
   diesel gensets with radiator packs, exhausts and plinth grates, under beamed
   ceiling with light strips). Raycast hits group by genset mesh-name families, so a
   click selects the whole set. Free-orbit room viewer, clamped in front of the wall. */
(function(){
const ESM='https://cdn.jsdelivr.net/npm/three@0.160.0';
const GLB='uploads/3d/generator-room.glb';
const POSTER='uploads/3d/generator-room-hero.png';
const SEC={
  G1:{name:'GENSET 1',type:'Standby diesel generator',devices:'Engine-alternator set · radiator & exhaust'},
  G2:{name:'GENSET 2',type:'Standby diesel generator',devices:'Engine-alternator set · radiator & exhaust'},
  G3:{name:'GENSET 3',type:'Standby diesel generator',devices:'Engine-alternator set · radiator & exhaust'}
};
const IDS=Object.keys(SEC);
const MAP=[
  [/^(GEN_AI_Genset$|G1_)/,'G1'],
...
class LMGenViewer extends HTMLElement{
  connectedCallback(){
  ...
  _fallback(e){
    console.error('Generator viewer:',e);
  ...
  _init(THREE,GLTFLoader,RoomEnvironment){
    ...
    new GLTFLoader().load(GLB,(gltf)=>{
      ...
      const lm=root.getObjectByName('GEN_Lights');
      if(lm){
        let found=false;
...
if(!customElements.get('lm-gen-viewer'))customElements.define('lm-gen-viewer',LMGenViewer);
})();
